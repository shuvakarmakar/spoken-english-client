...
import UpdateClass from "./UpdateClasses/UpdateClases";
...
const AllAddedClasses: React.FC = () => {
  ...
  useEffect(() => {
    if (user?.email) {
      ...
      fetch(
        `https://spoken-english-server-xi.vercel.app/added-courses-by-instructor/${user.email}`
      )
  ...
  const handleDelete = (id: string) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        // DeleteUsers
        fetch(
          `https://spoken-english-server-xi.vercel.app/deleteCourse/${id}`,
          {
            method: "DELETE",
          }
        )
  ...
  // console.log('Courses state:', courses); // Log courses state
    ...
    // handle course editing and updating
  ...
  const closeModal = () => {
    ...
    const handleEditclases = (id: string) => {
        openModal()
        setId(id);
  ...
  return (
    <>
              ...
              <tbody>
                {courses.map((course) => (
                  <tr key={course._id} className="text-white">
                    ...
                    <td onClick={() => handleEditclases(course._id)}>
                      <span>
                        {" "}
                        <BiEdit
      ...
      {isModalOpen && <UpdateClass id={id} isOpen={isModalOpen} onClose={closeModal} />}
    </>
  );
};
